...
import { getVideoPlayerOptions } from "../../src/lib/video-player.js";
  ...
  describe("getVideoPlayerOptions", () => {
    describe("Basic", () => {
      it("should return create an options object with minimal config", () => {
        ...
        const expectedOptions = {
          ...
          showLogo: true,
          transformation: [{ quality: "auto" }, undefined],
          width: "1620",
        };
      ...
      it("should configure custom quality", () => {
        ...
        expect(getVideoPlayerOptions(options, config)).toMatchObject({
          transformation: [{ quality: options.quality }, undefined],
        });
      });
    });
    ...
    describe("Customization", () => {
      ...
      it("should set a custom poster using constructCloudinaryUrl options syntax with public ID inherited", () => {
        ...
        expect(playerOptions.posterOptions?.publicId).toContain(
          `https://res.cloudinary.com/${config.cloud.cloudName}/video/upload/e_tint:${options.poster.tint}/f_auto:image/q_auto/v1/${options.src}`,
        );
      });
      ...
      it("should set a custom poster using constructCloudinaryUrl options syntax with custom thumb public ID", () => {
        ...
        expect(playerOptions.posterOptions?.publicId).toContain(
          `https://res.cloudinary.com/${config.cloud.cloudName}/image/upload/e_tint:${options.poster.tint}/f_auto/q_auto/v1/${options.poster.src}`,
        );
      });
    });
  });
