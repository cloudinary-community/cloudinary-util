...
import { constructCloudinaryUrl } from "../../src/lib/cloudinary.js";
  ...
  describe("constructCloudinaryUrl", () => {
    ...
    describe("cropping, resizing", () => {
      it("should create a Cloudinary URL with a width, height, and custom crop", () => {
        ...
        expect(url).toContain(
          `https://res.cloudinary.com/${cloudName}/image/${deliveryType}/c_${crop},w_${width},h_${height},g_auto/f_auto/q_auto/v1/${publicId}`,
        );
      });
      ...
      it("should create a Cloudinary URL with a aspect ratio, custom crop, zoom, and default gravity", () => {
        ...
        expect(url).toContain(
          `https://res.cloudinary.com/${cloudName}/image/${deliveryType}/c_${crop},ar_${aspectRatio},g_auto,z_${zoom}/f_auto/q_auto/v1/${publicId}`,
        );
      });
    });
    ...
    describe("deliveryType", () => {
      ...
      it("should create a Cloudinary URL from a Cloudinary source with spaces and commas", () => {
        ...
        expect(url).toContain(
          `https://res.cloudinary.com/${cloudName}/image/${deliveryType}/f_auto/q_auto/v1/${expectedId}?_a`,
        );
      });
    });
      ...
      it("should include an analytics ID at the end of the URL", () => {
        ...
        const url = constructCloudinaryUrl({
          ...
          analytics: {
            ...
            techVersion: "1.2.3",
            product: "B",
            // TODO: Colby IAnalyticsOptions requires feature but was not present
            feature: "",
          },
        });
    ...
    describe("preserveTransformations", () => {
      it("should preserve transformations with additional properties", () => {
        ...
        expect(url).toContain(
          `image/upload/${transformations.join("/")}/o_${opacity}/e_shear:${shear}/v${version}/${publicId}`,
        );
      });
    });
    ...
    describe("rawTransformations", () => {
      it("should apply rawTransformations to the beginning of the URL", () => {
        ...
        const rawTransformations = [
          "c_crop,h_359,w_517,x_1483,y_0/c_scale,h_359,w_517",
        ];
        const url = constructCloudinaryUrl({
          options: {
            src,
        ...
        expect(url).toContain(
          `image/upload/${rawTransformations.join("/")}/f_auto/q_auto/v1/${src}`,
        );
      });
      ...
      it("should not apply f_auto,q_auto if exists in raw transformations", () => {
        ...
        expect(url).toContain(
          `image/upload/${rawTransformations.join("/")}/v1/${src}`,
        );
      });
      ...
      it("should not apply f_auto if exists in raw transformations with / joined segments", () => {
        ...
        expect(url).toContain(
          `image/upload/${rawTransformations.join("/")}/c_limit,w_${width}/q_${quality}/v1/${src}`,
        );
      });
    });
    ...
    it("should apply effects by array", () => {
      ...
      expect(url).toContain(
        `${assetType}/upload/o_${opacity},e_shear:${shear}/e_cartoonify:${cartoonify},e_gradient_fade,r_${radius}/f_auto/q_auto/v1/${src}`,
      );
    });
